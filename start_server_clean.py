...
def main():
    ...
    try:
        ...
        def generate_image():
            try:
                ...
                if not generated_image_path:
                    print("⚠️ OpenRouter生成失败，使用本地生成器...")
        ...
        def api_status():
            ...
            try:
                # 检查OpenRouter API状态
                openrouter_status = openrouter_generator.test_connection()
                ...
                return jsonify({
                    'openrouter': {
                        'configured': openrouter_status['success'],
                        'status': 'OK' if openrouter_status['success'] else 'ERROR',
                        'message': openrouter_status.get('message', openrouter_status.get('error', ''))
                    },
                    'bytedance_video': {
                    ...
                    'overall_status': 'ready' if openrouter_status['success'] else 'fallback',
                    'message': 'OpenRouter图像生成 + ByteDance视频生成 + 本地回退',
                    'timestamp': datetime.now().isoformat()
                })
            except Exception as e:
                return jsonify({
